using NBitcoin.DataEncoders;
using NBitcoin.Protocol;
using System;
using Xunit;

namespace NBitcoin.Zcash.Tests
{
    public class ZcashNetworksTests
    {
        [Fact]
        public void Test1()
        {
			ZcashNetworks.Register();

            var k = new Key();
            var s = k.GetWif(ZcashNetworks.Testnet).ToString();
            var a = k.PubKey.GetAddress(ZcashNetworks.Testnet).ToString();

			//BitcoinAddress.Create("LQQGtMCw3KgdpZHLzqysfYbqFpzgnigNjM", ZcashNetworks.Mainnet);
			//BitcoinAddress.Create("ltc1q7ggs95p9yhh8lf4fy2ek5cv7uusfstsfulvc3u", ZcashNetworks.Mainnet);
			//BitcoinAddress.Create("tltc1qu5gkdjj5zxakj2kcnyc77djsqqft4pvux4krzxplscurn8mqhlkqt9qzmr", ZcashNetworks.Testnet);


			//var ex = Assert.Throws<Bech32FormatException>(() => BitcoinAddress.Create("tltc1qu4gkdjj5zxakj2kcnyc77djsqqft4pvux4krzxplscurn8mqhlkqt9qzmr", ZcashNetworks.Testnet));
			//Assert.Equal(7, ex.ErrorIndexes[0]);

			//Assert.Equal(ZcashNetworks.Testnet, BitcoinAddress.Create("tltc1qu5gkdjj5zxakj2kcnyc77djsqqft4pvux4krzxplscurn8mqhlkqt9qzmr").Network);
			//Assert.Equal(ZcashNetworks.Mainnet, BitcoinAddress.Create("ltc1q7ggs95p9yhh8lf4fy2ek5cv7uusfstsfulvc3u").Network);
			//Assert.Throws<FormatException>(() => BitcoinAddress.Create("tltc1qu5gkdjj5zxakj2kcnyc77djsqqft4pvux4krzxplscurn8mqhlkqt9qzmr", ZcashNetworks.Mainnet));

			//var header = new BlockHeader("01000000f615f7ce3b4fc6b8f61e8f89aedb1d0852507650533a9e3b10b9bbcc30639f279fcaa86746e1ef52d3edb3c4ad8259920d509bd073605c9bf1d59983752a6b06b817bb4ea78e011d012d59d4");
			//Assert.Equal("adf6e2e56df692822f5e064a8b6404a05d67cccd64bc90f57f65b46805e9a54b", header.ToString());

			//Assert.Equal("0000000110c8357966576df46f3b802ca897deb7ad18b12f1c24ecff6386ebd9", ZcashNetworks.Mainnet.Consensus.GetPoWHash(header).ToString());

			//var b = new Block(Encoders.Hex.DecodeData(""));
			//Assert.True(b.CheckMerkleRoot());
			//Assert.True(b.Header.CheckProofOfWork(ZcashNetworks.Mainnet.Consensus));


			//var genesis = ZcashNetworks.Mainnet.GetGenesis();
			//var b1 = new Block(Encoders.Hex.DecodeData(""));

			//var b2 = new Block(Encoders.Hex.DecodeData(""));

			//var chain = new ConcurrentChain(ZcashNetworks.Mainnet);
			//chain.SetTip(b1.Header);
			//chain.SetTip(b2.Header);
			//Assert.Equal(2, chain.Height);
			//Assert.True(chain.Validate(ZcashNetworks.Mainnet));
		}
    }
}
